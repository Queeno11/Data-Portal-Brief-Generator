...
df.stage_life = pd.Categorical(df.stage_life, categories=ordered_categories, ordered=True)
# Replace gender with sheetnames
df.gender = df.gender.replace({'Total': 'HCCI', 'Male': 'HCCI - Male', 'Female': 'HCCI - Female'})

###### Read metadata ######
metadata = pd.read_stata(rf"{data_processed}\metadata_processed.dta")
metadata = metadata[metadata['rank'] != 0] # Drop indicators that are already included in the HCI

## Format metadata ##
sources = metadata[['stage_life', 'dimension', 'name', 'description', 'source', 'code', 'download_link']]
sources.columns = ['Stage of life', 'Dimension', 'Human Capital Complementary Indicator Name', 'Definition', 'Source', 'Source code (when available)', 'Source URL']
...
sources["Stage of life"] = pd.Categorical(sources["Stage of life"], categories=ordered_categories, ordered=True)
sources = sources.sort_values(['Stage of life', 'Dimension', 'Human Capital Complementary Indicator Name'])
...
for wbcode, countryname in tqdm(country_array): 
    ...
    with pd.ExcelWriter(rf"{excels}/HCI_Data_July_2023_{wbcode}.xlsx", engine='xlsxwriter') as writer:  
        ...
        for sheetname in ['HCCI', 'HCCI - Male', 'HCCI - Female', 'Sources']:
            ...
            ### Add WB logo
            worksheet.insert_image("H1", fr"{portal}\Sources\WBG_logo.png", {"x_scale": 0.26, "y_scale": 0.26})

            ### Format excel
            # Set background color of cells
            worksheet.set_column(0, number_of_cols+10,8.43 , sheet_format) # 8.43 is default width
            ...
            if sheetname == "Sources":
                ...
                worksheet.set_column(first_excel_col+2, first_excel_col+2, 45, values_format) # Name
                worksheet.set_column(first_excel_col+3, first_excel_col+3, 60, values_format) # definition
                worksheet.set_column(last_excel_col-1, last_excel_col-1, 150, values_format) # url
                ...
                for col_num, col in enumerate(to_excel.columns):
                    worksheet.write(first_excel_row, col_num+startcol, col, header_format)
                    # Overwrite both the value and the format of each value cell
                    for row_num, value in enumerate(to_excel[col]):
                        worksheet.write(first_excel_row+row_num+1, startcol+col_num, to_excel.loc[row_num, col], sources_format)
                ...
                # Overwrite both the value and the format of each header cell -- text_wrap only works with write...
                relevant_cols = to_excel.columns.values[1:] # Remove year from list of cols
                for col_num, value in enumerate(relevant_cols):
                    ...
                    worksheet.write(first_excel_row, col_num+startcol+1, value, header_format)
                
                # Overwrite both the value and the format of each index cell
                for row_num, value in enumerate(to_excel['year']):
                    worksheet.write(row_num+startrow+1, first_excel_col, value, index_format)
                
                # Columna de año - vacia
                worksheet.write(first_excel_row, first_excel_col, "Year", box_format)

            # Set outside borders
            topFormat = workbook.add_format({'top': 2, 'bg_color': "#FFFFFF",})
            leftFormat = workbook.add_format({'left': 2, 'bg_color': "#FFFFFF",})
            rightFormat = workbook.add_format({'right': 2, 'bg_color': "#FFFFFF",})
            ...
            for row in range(first_excel_row, last_excel_row+1):
                if (row != first_excel_row) & (sheetname != "Sources"):
                    ...
                    worksheet.write(row, first_excel_col-1, '', rightFormat) # right border of excel
                worksheet.write(row, last_excel_col, '', leftFormat) # right border of excel
                
            for col in range(first_excel_col, last_excel_col):
                worksheet.write(last_excel_row+1, col, '', topFormat) # bottom border of excel

        # Graphs_HCCI sheet
        worksheet = workbook.add_worksheet("Graphs_HCCI")
        worksheet.set_column(0, 60, 8.43 , sheet_format) # 8.43 is default width
        worksheet.write(2, 1, "Human Capital Complementary Indicators (HCCI): Graphs", title_format)
        worksheet.write(2, 16, "Human Capital Index (HCI): Graphs", title_format)

        ### Add Graphs as images
        worksheet.insert_image("B5", fr"{portal}\Graphs\p2_AUS_stages.jpg", {"x_scale": 0.5, "y_scale": 0.5})
        worksheet.insert_image("Q5", fr"{portal}\Graphs\p1_AUS_all.jpg", {"x_scale": 0.5, "y_scale": 0.5})
